...
BEGIN
	...
	/*
    @a from the first test:
	SELECT
		"AVG_EXEC_PRICE" query_name,
		(SELECT AVG(execution_price) FROM `execution`) val
	UNION
		SELECT
		"COUNT_EXECUTIONS" query_name,
		(SELECT COUNT(execution_id) FROM `execution`) val
	UNION
		SELECT
		"MIN_ORDER_DATE" query_name,
		(SELECT MIN(date_placed) FROM `orders`) val
	UNION
		SELECT
		"SUM_ORD_QTY" query_name,
		(SELECT SUM(order_qty) FROM `orders`) val
	ORDER BY 1
	*/
